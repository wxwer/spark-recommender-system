...
if __name__ == "__main__":
    ...
    np.random.seed(seed)
    random.seed(seed)
    ml100k_file=r'D:\files\code\RecommendSystem\data\ml-latest-small\ml_100k_feat.csv'
    data = pd.read_csv(ml100k_file)
    ...
    print(feat_sizes)
    with open('feat_sizes.pkl','wb') as f:
        pickle.dump(feat_sizes,f)
    ...
    model.fit(train_model_input, train[target].values , test_model_input , test[target].values ,batch_size=1024, epochs=2, verbose=1)
    ...
    torch.save(model.state_dict(),'./ml100k_checkpoint.h5')
    
    model2 = Deepfm(feat_sizes ,sparse_feature_columns = sparse_features,dense_feature_columns = dense_features,
                   dnn_hidden_units=[400,400,400] , dnn_dropout=0.3 , ebedding_size = 8 ,
                   l2_reg_linear=1e-3, device=device)
    model2.load_state_dict(torch.load('./ml100k_checkpoint.h5'))
    print("加载后",model2.predict(pred_model_input),)
